...
public class UIModel {
	...
	public static int GAME_ATTRIBUTE_PIXEL_DENSITY_Y = 10;

	// 游戏等级提升因数
	public static int GAME_ATTRIBUTE_LEVEL_UP_FACTOR = 40;

	// 重力速度(即主角离开踏板后的y方向速度)
	public static int GAME_ATTRIBUTE_GRAVITY_VELOCITY = 5 * GAME_ATTRIBUTE_PIXEL_DENSITY_Y;
	...
	// 主角的长度和宽度
	public static int ROLE_ATTRIBUTE_WIDTH = 30;
	public static int ROLE_ATTRIBUTE_HEITH = 34;

	// 主角帧刷新间隔
	public static final int ROLE_ATTRIBUTE_FRAME_DELAY = 2;

	// 主角最大生命值
	public static final int ROLE_ATTRIBUTE_HP_MAX = 4;
	...
	// 加分物品在屏幕上的滞留时间
	public static final int FOOD_ATTRIBUTE_DELAY_TIME = 180;

	// 加分物品的大小
	public static int FOOD_ATTRIBUTE_IMAGE_SIZE = 24;

	// 踏板的长宽
	public static int BORDER_ATTRIBUTE_IMAGE_HEITH = 40;
	public static int BORDER_ATTRIBUTE_IMAGE_WIDTH = 130;

	// 踏板偏向速度
	public static int BOARD_ATTRIBUTE_LEFT_VELOCITY = -4;
	public static int BOARD_ATTRIBUTE_RIGHT_VELOCITY = 4;

	// 不稳定踏板滞留因数(可滞留时间=滞留因数*帧刷新间隔)
	public static int BOARD_ATTRIBUTE_UNSTABLE_DELAY_FACTOR = 10;
	...
	private static int mFootboardSpaceFactor = 120 * GAME_ATTRIBUTE_PIXEL_DENSITY_Y;

	// 移动间隔计算器
	private int mFootboardSpaceCounter = 0;

	// 踏板移动速度
	private static int mFootboartVelocity = -5 * GAME_ATTRIBUTE_PIXEL_DENSITY_Y;
	...
	public static void initDate(Context context) {
		...
		GAME_ATTRIBUTE_PIXEL_DENSITY_Y = ToolUtil.dip2px(context, 10);

		FOOD_ATTRIBUTE_IMAGE_SIZE = ToolUtil.dip2px(context, 30);

		BORDER_ATTRIBUTE_IMAGE_HEITH = ToolUtil.dip2px(context, 30);
		BORDER_ATTRIBUTE_IMAGE_WIDTH = ToolUtil.dip2px(context, 120);

		ROLE_ATTRIBUTE_WIDTH = ToolUtil.dip2px(context, 30);
		ROLE_ATTRIBUTE_HEITH = ToolUtil.dip2px(context, 34);

		BOARD_ATTRIBUTE_UNSTABLE_DELAY_FACTOR = ToolUtil.dip2px(context, 10);
		GAME_ATTRIBUTE_GRAVITY_VELOCITY = ToolUtil.dip2px(context, 3)
				* GAME_ATTRIBUTE_PIXEL_DENSITY_Y;

		BOARD_ATTRIBUTE_LEFT_VELOCITY = -ToolUtil.dip2px(context, 4);
		BOARD_ATTRIBUTE_RIGHT_VELOCITY = ToolUtil.dip2px(context, 4);

		mFootboardSpaceFactor = ToolUtil.dip2px(context, 120)
				* GAME_ATTRIBUTE_PIXEL_DENSITY_Y;
		mFootboartVelocity = -ToolUtil.dip2px(context, 3)
				* GAME_ATTRIBUTE_PIXEL_DENSITY_Y;
	}

	public UIModel(Context context, ScreenAttribute screenAttribute,
			int addVelocity) {
	...
	public void handleMoving(float angleValue) {
		...
		mRoleVelocityX = ToolUtil.dip2px(context, mRoleVelocityX);
	}
	...
	private void handleRoleAction() {
		...
		if (mRoleVelocityY < mFootboartVelocity) {
			mRoleVelocityY += ToolUtil.dip2px(context, 3);
		} else {
			mRoleVelocityY = GAME_ATTRIBUTE_GRAVITY_VELOCITY;
		}
		updateRoleStatus(ROLE_STATUS_FREEFALL);
